...
- **Memory is allocatee on actor stacks, not actor heaps.** **Memory allocation happens on each actor’s stack**—storage is **stack-shaped and flexibly sized**, with **no default per-actor heap** for long-lived data. Lifetimes follow **calls and frames**, which keeps memory easy to reason about and **wipes out** typical **heap-style** mistakes (use-after-free, double-free, leaks, etc.) **without** a garbage collector. **Sharing stays message-shaped** and execution stays **predictable**. See the [language specification](compiler/silica-compiler/design_documents/silica-specification.md) (memory model, actors).
