...
- Explicit effect declarations: `proc[device_io, concurrency]`
- Clear block delimiters: `do...end` for process blocks, `{...}` for case expressions
...
- All side effects are declared in function signatures: `proc[device_io, mem(normal)]`
- Effect requirements propagate through call chains
...
```silica
fn main() -> int64 proc[device_io, concurrency] {
    do
...
**Example - Function literal with device_io effect:**
```silica
fn main() -> int proc[concurrency, device_io] {
        echo_ref: actor_ref <- spawn(
        EchoState { received: 0 },
        fn(msg: Response, state: EchoState) -> EchoState proc[device_io] {
            print_string("Received message: ");
...
#### 9.3.1 Function Effects
Functions track effects of their body. **All function signatures must explicitly declare all effects** - Silica does not perform effect inference. Effects must be declared in the function signature using the `proc[effect_list]` syntax.

**Explicit Effect Requirement:**

Every function that uses effects must declare them explicitly in its signature:
...
1. **Pure Functions**: Functions with no effects use `proc[]` (empty effect list):
   ```
   fn add(x: int64, y: int64) -> int64 proc[] {
       x + y
   }
   ```
...
**Example 6: Effect Composition in Do Expressions**
...
1. All effects used in the function body are declared in the signature
2. All effects required by called functions are declared
...
Function uses mem(normal) effect but does not declare it in signature.
...
Function calls helper() which requires mem(normal) effect, but caller() does not declare it.
Required effects: [mem(normal)]
Declared effects: []
See specification: spec:§9.3.1
...
Function literal uses device_io effect but does not declare it in signature.
...
Function uses [mem(normal), device_io] effects but does not declare them in signature.
Required effects: [mem(normal), device_io]
Declared effects: []
See specification: spec:§9.3.1
...
- **E3001**: Missing effect declaration in function signature
...
**Effect Composition in Do-Expressions:**
...
All effects are preserved in function signatures - no effect erasure occurs.
...
- All effects in a function body are declared in its type signature
...
**Note**: Binding creates process values but does not execute them. Processes are executed using the `spawn()` function. The last expression in a `do...end` block without a semicolon is the return value.
...
**Note**: The last expression in a `do...end` block must not have a semicolon. The absence of a semicolon declares that expression as the return value.
