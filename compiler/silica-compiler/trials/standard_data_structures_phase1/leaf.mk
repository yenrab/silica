...
#   trial_compile_fail_*        expected compile failure (error_enforcement leaf)
...
assembly: silica.config
	...
	@if [ ! -x "$(SILICA_COMPILER)" ]; then \
		...
		echo "      build with: make -C $(THIS_DIR)../../../src"; \
		exit 1; \
	fi
	@$(INTEGRATE_PRE_CLEAN)
...
record-positive-golden: silica.config
	...
	@if [ ! -x "$(SILICA_COMPILER)" ]; then \
		echo "FAIL: build silica-compiler first: make -C $(THIS_DIR)../../../src"; \
		exit 1; \
	fi
	@$(INTEGRATE_PRE_CLEAN)
	@cd "$(THIS_DIR)" && "$(SILICA_COMPILER)"
