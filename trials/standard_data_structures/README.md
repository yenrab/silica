...
Compile-fail constructor/trait/type checks live under
[`../error_enforcement_addition/standard_data_structures/`](../error_enforcement_addition/standard_data_structures/)
(included via top-level `error_enforcement_addition` integrate).
...
| **Compile failure** | `trial_compile_fail_*.silica` | `.golden_fail` | `error_enforcement_addition/standard_data_structures/` (isolated compile) |
...
```bash
make record-golden   # capture .ascomp and .scout for positive leaves
```

Compile-fail `.golden_fail` capture:

```bash
make -C ../error_enforcement_addition/standard_data_structures record-golden
```
